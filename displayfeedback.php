...
<section class="container py-5">
    ...
    <div class="row">
        ...
        $feedback_query = "SELECT * FROM feedback ORDER BY fid";
        ...
        if (mysqli_num_rows($feedback_result) > 0) {
            while ($feedback_row = mysqli_fetch_assoc($feedback_result)) {
                ...
                echo "<h5 class='card-title'>User: {$feedback_row['username']}</h5>";
                echo "<p class='card-text'>Feedback: {$feedback_row['feedback']}</p>";
                echo "</div>";
                echo "</div>";
                echo "</div>";
            }
            ...
            echo "<div class='col-12 text-center'><p>No feedback available.</p></div>";
        }
